...
class GradeSubject:
    ...
    def get_grade(self, brain):
        ...
        for i in range(len(self.questions)):
            ...
            if len(self.instream_questions) <= 0:  # Use qs from text
                brains_answer = brain.process_input(self.quick_expand_input_to_fit_brain_N(self.questions[i],N))
            else:  # Use pre-loaded input
                brains_answer = brain.process_input(self.instream_questions)
            delta = 0
            penalty = 0

            #print(brains_answer)

            # Check to see if answer was found
            ...
            # Check to see if more words were returned other than correct
            for row_answer in brains_answer:
                if row_answer:
                    for brains_answer_token in row_answer:
                        correct_word = 0
                        q_word = brains_answer_token[0]
                        for a_token in a_w_tags:
                            a_word = a_token[0]
                            if a_word == q_word:
                                correct_word = 1
...
class evolution_optimization:
    ...
    def roulette_wheel(self, pop_score, bonus = 1):
    ...
    def next_generation(self, wheel):
        ...
        if True:
            ...
            for j in range(len(pop_score)):
                if abs(pop_max - pop_score[j]) < 0.001:
                    ...
                    self.hall_of_fame.append(entry_hall_of_fame)
                    break;
            wheel = self.roulette_wheel(pop_score, bonus = 100)
            #print(wheel)
            nextgen = self.next_generation(wheel)
            #print(len(nextgen))
            self.population.organism = nextgen
...
if orthogonal_testing:
    ...
    for ft in L4_Orth:
        ...
        print("completed trial: total time:\t" + elapsed_time.__str__() + "\tpop_mean:\t" + pop_mean.__str__())
...
if testing:
    ...
    def ready_brain(grade_filename, brain_filename):
        y = GradeSubject(grade_filename)
        blankUtility = CoreUtility([[]])
        brain = CoreUtility(blankUtility.load_cortex(brain_filename))
        return y, brain

    # Load brains
    qs_and_as_filenames = ["subject_answer_key.txt", "object_answer_key.txt", "action_answer_key.txt"]
    brain_filenames = ["subject_brain.p", "object_brain.p", "action_brain.p"]
    graders = []
    brains = []
    for i in range(len(brain_filenames)):
        grader, brain = ready_brain(qs_and_as_filenames[i],brain_filenames[i])
        graders.append(grader)
        brains.append(brain)

    def test_score(grader, brain):
        print("testing brain...")
        out_of = grader.max_score()
        score = grader.get_grade(brain)
        print(score.__str__() + " / " + out_of.__str__())

    for i in range(len(brains)):
        #test_score(graders[i], brains[i])
        print("need to optimize sub-brains")

    # Note: Brains need to be optimized
    # Create stage 2 brain (placeholder at this point)
    id_brain = IdentifyWordFunction()
    brain_s2 = id_brain.basic_cortex(300, 10, [0, 1, 2, 0, 1, 2, 0, 1, 2, 1])
    rh_brain = CoreUtility(brain_s2)

    # Stack sub-brians in column
    brains[0].append_neuron_array(brains[1].neuron_array, 'down')
    brains[0].append_neuron_array(brains[2].neuron_array, 'down')
    print('dimensions of stacked brain')
    print('N ' + brains[0].N.__str__())
    print('Columns ' + brains[0].Columns.__str__())

    # Stack sub-brains against right brain, creating stage-2 brain
    # Eventually....Currently, need to optimize right brain
    # For each line of input, get array of output from left hand brain. Save.
    lh_brain = brains[0]
    input_getter = GradeSubject('stage2grader.txt')
    instream_input = []
    for q_input in input_getter.questions:
        lines_input = input_getter.quick_expand_input_to_fit_brain_N(q_input, 300)
        lines_output = lh_brain.process_input(lines_input)
        instream_input.append(lines_output)
    rh_grader = input_getter;
    rh_grader.instream_questions = instream_input
    rh_grader.questions.clear()
    print(rh_grader.get_grade(rh_brain))
    #lh_brain.process_input(input_x_300)
